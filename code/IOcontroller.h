...
public:
    ...
    static std::string BuildOutFileName(const std::string graphName, const std::string algName, const int seedsize,
                                        const std::string probDist, const float probEdge, const std::string skewType)
    {
        if (probDist == "skewed")
        {
            return graphName + "_" + "k"  + "_radeim_"  + skewType;// + "_" + std::
             //     + std::to_string(probEdge);
        }
        if (probDist == "uniform")
        {
            return graphName + "_" + "k"  + "_radeim_"  + probDist;// + "_" + std::
             //     + std::to_string(probEdge);
        }

        return graphName + "_" + "k_radeim_"  + probDist;
           //       + std::to_string(probEdge);
    }
...
*/
	...
	static void write_result(const std::string& outFileName, const TResult& resultObj, const std::string& outFolder, double epsilon, double ita, const std::string probDist, double probEdge)
	{
		const auto approx = resultObj.get_approximation();
		const auto runTime = resultObj.get_running_time();
		const auto influence = resultObj.get_influence();
		...
		std::cout << "   --------------------" << std::endl;
		mkdir_absence(outFolder.c_str());
		std::ofstream outFileNew(outFolder + "/" + outFileName, std::ios::app);
		if (outFileNew.is_open())
		{
			outFileNew << seedSize << "\t";
			outFileNew << epsilon << "\t";
			...
			outFileNew << approx << "\t";
			outFileNew << runTime << "\t";
			outFileNew << influence << "\t";
			outFileNew << influenceOriginal << "\t";
			outFileNew << RRsetsSize << "\t";
			outFileNew << probDist << "\t";
			outFileNew << probEdge << "\t" << std::endl;
			outFileNew.close();
		}
	}
    /// Print the seeds
